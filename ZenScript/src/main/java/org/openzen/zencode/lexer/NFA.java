...
public class NFA
{
	...
	private Partial processPartial(CharStream stream)
	{
		if (stream.optional('(')) {
			...
			for (int i : processCharList(stream).toArray()) {
	...
	private TIntSet processCharList(CharStream stream)
	{
		boolean invert = stream.optional('^');
		TIntSet base = new TIntHashSet();
		do {
	 ...
	 * of characters. */
	private void processCharPartial(TIntSet out, CharStream stream)
	{
		if (stream.optional('.'))
			for (int i = 0; i <= 256; i++) {
				out.add(i);
